...
class Main(tk.Tk):
	...
	def __init__(self, convert):
		...
		self.CurrencyConverter = convert
		...
		# Create data label
		self.data_label = Label(self, text=f'{dt.datetime.now():%A, %B, %d, %Y}', bg='#3A3B3C', fg='white', font=('calibri', 10))
		self.data_label.place(x=0, y=400, anchor='sw')
		...
		self.currency_var1 = StringVar(self)
		self.currency_var2 = StringVar(self)
		self.currency_var1.set('USD')
		self.currency_var2.set('INR')


		self.currency_combobox1 = ttk.Combobox(self, width=20, textvariable=self.currency_var1, values=list(self.CurrencyConverter.rates.keys()), state='readonly')
		self.currency_combobox1.place(x=200, y=170, anchor='center')

		self.currency_combobox2 = ttk.Combobox(self, width=20, textvariable=self.currency_var2, values=list(self.CurrencyConverter.rates.keys()), state='readonly')
	...
	# Create a funciton to perform
	def processed(self):
		try:
			given_amount = float(self.amount_entry.get())
			given_base_currency = self.currency_var1.get()
			given_des_currency = self.currency_var2.get()
